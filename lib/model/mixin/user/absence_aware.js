...
module.exports = function(sequelize) {
  ...
  this.promise_my_leaves = function(args) {
    ...
    let year = args.year || moment.utc()

    // Ensure year is a valid moment object, and extract year number if needed
    if (!moment.isMoment(year)) {
      year = moment.utc(year)
    }

    // Validate the year moment object
    if (!year.isValid()) {
      console.warn('Invalid year parameter in promise_my_leaves, using current year')
      year = moment.utc()
    }
    ...
    // Case when there are start and end date defined
      ...
      const startMoment = moment.utc(dateStart)
      const endMoment = moment.utc(dateEnd)

      if (!startMoment.isValid() || !endMoment.isValid()) {
        console.warn('Invalid dateStart or dateEnd in promise_my_leaves:', {
          dateStart,
          dateEnd,
          startValid: startMoment.isValid(),
          endValid: endMoment.isValid()
        })
        // Skip this filter if dates are invalid
      } else {
        where_clause.push({
          [Op.or]: {
            date_start: {
              $between: [
                ...
                endMoment.endOf('day').format('YYYY-MM-DD HH:mm')
              ]
            },
            date_end: {
              $between: [
            ...
            $and: {
              date_start: {
                $lte: startMoment.startOf('day').format('YYYY-MM-DD')
              },
              date_end: {
                $gte: endMoment.endOf('day').format('YYYY-MM-DD')
              }
            }
          }
        })
  ...
  this.promise_my_leaves_for_calendar = function(args) {
    let year = args.year || this.company.get_today()

    // Ensure year is a valid moment object
    if (!moment.isMoment(year)) {
      year = moment.utc(year)
    }
    ...
    // Validate the year moment object
    if (!year.isValid()) {
      console.warn('Invalid year parameter in promise_my_leaves_for_calendar, using current year')
      year = this.company.get_today()
    }

    let yearStart = moment.utc(year).startOf('year')
    let yearEnd = moment.utc(year).endOf('year')

    // Validate the dates before using them
    if (!yearStart.isValid() || !yearEnd.isValid()) {
      console.warn('Invalid year dates in promise_my_leaves_for_calendar')
      yearStart = moment.utc().startOf('year')
      yearEnd = moment.utc().endOf('year')
    }
        ...
        [Op.or]: {
          date_start: {
            [Op.between]: [
              ...
              yearEnd.format('YYYY-MM-DD HH:mm')
            ]
          },
          date_end: {
            [Op.between]: [
