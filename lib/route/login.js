...
// Safely parse registration option from config
// Handles boolean, string, or undefined values
function get_registration_enabled() {
  const registrationConfig = config.get('options:registration')
  if (registrationConfig === undefined) {
    return true // default to enabled
  }
  if (typeof registrationConfig === 'boolean') {
    return registrationConfig
  }
  if (typeof registrationConfig === 'string') {
    try {
      return JSON.parse(registrationConfig)
    } catch (e) {
      // If parsing fails, treat as truthy string (only 'false' string is false)
      return registrationConfig.toLowerCase() !== 'false'
    }
  }
  return true // default
}
...
module.exports = function(passport) {
  ...
  router.get('/login', (req, res) => {
    res.render('login', {
      ...
      allow_create_new_accounts: get_registration_enabled(),
      title: 'Login  | TimeOff',
  ...
  router.get('/register', (req, res) => {
    // Disable new accounts if registration is disabled
