...
// Configure SMTP transport with settings from config
// Support both camelCase (from JSON) and snake_case (from env vars)
const smtpConfig = config.get('smtp')
const getValue = (obj, camelKey, snakeKey, defaultValue) =>
  obj[camelKey] !== undefined ? obj[camelKey] : obj[snakeKey] !== undefined ? obj[snakeKey] : defaultValue

const transporter = nodemailer.createTransport(
  smtpTransport({
    host: smtpConfig.host,
    port: smtpConfig.port,
    secure: getValue(smtpConfig, 'secure', 'secure', false),
    requireTLS: getValue(smtpConfig, 'requireTLS', 'require_tls', false),
    ignoreTLS: getValue(smtpConfig, 'ignoreTLS', 'ignore_tls', false),
    auth: smtpConfig.auth && smtpConfig.auth.user ? smtpConfig.auth : false,
    tls: {
      rejectUnauthorized: getValue(
        smtpConfig.tls || {},
        'rejectUnauthorized',
        'reject_unauthorized',
        true
...
Email.prototype.promise_leave_request_revoke_emails = function(args) {
  ...
  // Ensure leave object has all associations loaded before rendering
  return leave.reloadWithAssociates().then(leave => {
